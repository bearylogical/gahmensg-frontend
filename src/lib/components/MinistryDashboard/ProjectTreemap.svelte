  ...
  function parseTableData(data: any) {
    return data.children.map((d) => {
      return {
        ...
        percent_total: d.data.percentage,
        project_id: d.data?.data?.project_id ?? null,
      };
    });
  }
...
<Breadcrumb items={selectedZoomable?.ancestors().reverse() ?? []} class="pb-4">
  ...
  >
    <div class="text-left">
      <div class="text-2xl">
        {item.data.id == "/" ? "Projects" : formatTitle(item)}
      </div>
...
<div class=" mx-auto py-8">
  <div class="col-span-2 rounded-lg">
    {#if selectedZoomable}
      <div class="p-2">
        <ProjectDataTable data={tableData} />
      </div>
    {/if}
  </div>
</div>
