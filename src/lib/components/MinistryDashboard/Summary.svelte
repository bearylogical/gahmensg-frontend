...
<Card size="xl">
  <div class="mb-4 flex items-center gap-2">
    ...
    <Popover placement="bottom-start">
      ...
      >
        ...
        <p>
          Shows the top 5 personnel categories and projects in the selected
          ministry
        </p>
      </div>
    </Popover>
  ...
  >
            ...
            <div class="flex items-center justify-between">
              <div class="flex min-w-0 items-center">
                <div class="ml-3 flex flex-col">
                  <Tooltip.Root>
                    <Tooltip.Trigger>
                      <Tooltip.Root>
                                        <Tooltip.Trigger>
                                            <p
                                                class="block truncate max-w-[400px] font-medium text-gray-900 dark:text-white"
                                            >
                                                {project_title}
                                            </p>
                                        </Tooltip.Trigger>
                  ...
                  {#if parent_header !== ""}
                    <Badge
                      class="truncate ... max-w-[200px] hover:opacity-70"
                      rounded
              ...
              >
                ...
                <div class="text-xs font-light text-muted-foreground">
                  <span class={getArrowColor(perc_diff)}>
                    {getArrow(perc_diff)}{parseValue(perc_diff)}%</span
                  >
                  from {value_year - 1}
                </div>
              </div>
    ...
    <TabItem class="w-full">
      ...
      >
        {#each projectData as { project_title, project_id, parent_header, value_amount, perc_diff, value_year }}
          <li class="py-3 sm:py-4">
            <div class="flex items-center justify-between">
              <div class="flex min-w-0 items-center">
                <div class="ml-3">
                  <Tooltip.Root>
                    <Tooltip.Trigger>
                      <a
                        href="/budget/project?id={project_id}"
                  ...
                  {#if parent_header !== ""}
                    <Badge
                      ...
                      rounded
                      color={dynamicColorMap[parent_header]}
                    >
                      <span class="text-xs">
                        {parent_header}
                      </span>
