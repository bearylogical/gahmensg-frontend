...
export function getMostRecentYearData(data: ProjectRecord[]): ProjectRecord | undefined {
    if (data.length === 0) {
        return undefined;
    }
...
export function getPercentageDiff(data: ProjectRecord[]): number {
    ...
    if (!previousYear || previousYear.value_amount === undefined || mostRecentYear.value_amount === undefined) {
        return 0;
    }
    return (
...
export function getTopNData(data: ProjectRecord[], n: number): ProjectRecord[] {
    ...
    data.sort((a: ProjectRecord, b: ProjectRecord) => (b.value_amount || 0) - (a.value_amount || 0));
...
export function filterData(data: ProjectRecord[]): ProjectRecord[] {
    ...
    return data.filter((d: ProjectRecord) => {
        const found = res.find((r: [number | undefined, number | undefined]) => r[0] === d.value_year);
        return (
...
export function getMostRecentYearDataNoSum(data: ProjectRecord[]): ProjectRecord[] {
    const mostRecentYearData = getMostRecentYearData(data);
    if (!mostRecentYearData || mostRecentYearData.value_year === undefined) {
        return [];
    }
    return data.filter(
        (d: ProjectRecord) => d.value_year === mostRecentYearData.value_year,
    );
}
